...
import org.hspt.config.RequestMappingHandlerConfig;
import org.hspt.dao.jpa.ModuleDAO;
import org.hspt.dao.jpa.PermissionsDAO;
import org.hspt.dao.jpa.PermissionsResourceDAO;
import org.hspt.dao.jpa.ResourceDAO;
...
public class Pub_DBUtils {
    ...
    public void addApiToDB() {
        ...
        //添加模块信息
        HsptModule module = moduleDAO.findByModuleIdAndDr(module_id, BaseConstants.DATA_STATUS_OK);
        if (null == module) {
            module = new HsptModule();
        }
        ...
        for (RequestMappingInfo info : mappings) {
            ...
            if (moduleStr.startsWith(BaseConstants.CONTROLLER_BASE_PATH)) {
                ...
                //获取API信息
                HsptResource impResource = resourceDAO.findByModuleIdAndMethodPathAndMethodName(module_id, moduleStr, methodName);
                if (null == impResource) {
                    impResource = new HsptResource();
                }
                ...
                if (genPermissions && null != impResource.getPkResource()) {
                    ...
                    HsptPermissions impPermissions = permissionsDAO.findByPermissionsCodeAndDr(code, BaseConstants.DATA_STATUS_OK);
                    if (null == impPermissions) {
                        impPermissions = new HsptPermissions();
                    }
                    ...
                    //构建权限资源信息
                    if (null != impPermissions) {
                        HsptPermissionsResource permissionsResource = permissionsResourceDAO.
                        ...
                        if (null == permissionsResource) {
                            permissionsResource = new HsptPermissionsResource();
